...
import { States, Cities } from '../imports/api/states_and_cities.js';

if (Meteor.isClient) {

  const get_films_fields = function () {
  ...
  const get_films_filter = function (Session) {
  ...
  Template.admSessions.helpers({
    ...
    cities_options(films) {
      let filter = { has_screenings: true },
  ...
  Template.admSessions.onRendered = function () {
    // Deixa o mês ativado.
  ...
  Template.admSessions.events({
    'change .list-sessions': function (e) {
      const list = $(e.currentTarget).val();
    ...
    'change #ambassador-selector': function (e) {
      const ambassador = $(e.currentTarget).val();
    ...
    'click .btn-set-draft': function (e) {
      ...
      let film = film_and_screening.film,
    ...
    'click .btn-unset-draft': function (e) {
      const id = $(e.currentTarget).data('session-id');
      Meteor.call('setScreeningDraftStatus', id, false);
    },
    'click .csv-export'() {
      const screenings = this.screenings;
      const data = screenings.map(function (scr) {
        let d = moment(scr.date),
